...
class GeneticAlgorithmSolver:
    ...
    def generate_population(self, is_candidate_mode):
        ...
        for _ in range(self.pop_size):
            population.append(self.generate_chrom(is_candidate_mode))
        return population
    ...
    def evaluate_chrom(self, chrom):
        ...
        # block_factor = 1.
        # for col in chrom.T:
        #     reward = (len(np.unique(col)) / len(col)) ** (1/2)
        #     unique_count += col_factor * reward * len(col)
        #     max_score += col_factor * len(col)
        ...
        #         reward = (unique_count_block / 9) ** (1/2)
        #         unique_count += block_factor * reward * 9
    ...
    def solve(self, sudoku: Sudoku, is_candidate_mode=False, *args, **kwargs):
        """
        ...
        while epoch < self.max_epoch:
            P_epoch_selected = self.selection(P_epoch, P_epoch_scores, is_candidate_mode)
            P_epoch_mutated = self.crossover_mutation(P_epoch_selected, is_candidate_mode)
            ...
            print(f"Epoch: {epoch} best_global: {round(100*best_score_global, 2)}%, best_local: {round(100*best_score_local, 2)}%")
            ...
            if reset_condition == self.reset_condition_val:
                print(f"No improvement. Best solution:\n{best_chrom_global} \nGenerating new population.")
                P_epoch = self.generate_population(is_candidate_mode)
                P_epoch_scores = get_scores(P_epoch)
                reset_history.append((best_chrom_global, best_score_global))
                best_score_global = 0

        return best_chrom_global, best_score_global, np.array(best_score_per_epoch), reset_history
